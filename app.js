...
// Validação da conexão Supabase
if (!window.supabaseClient || typeof window.supabaseClient.from !== 'function') {
  throw new Error('supabaseClient.from não está disponível. Verifique config.js e a ordem dos scripts no index.html.');
}

// Helper para retornar string SVG de uso
const getIcon = (id) => `<svg class="icon"><use href="#icon-${id}"/></svg>`;
...
function statusBadge(status) {
  const map = {
    // Entregas
    'pendente':      ['badge-gray',   getIcon('clipboard') + ' Pendente'],
    'em_separacao':  ['badge-yellow', getIcon('package') + ' Em Separação'],
    'em_rota':       ['badge-blue',   getIcon('truck') + ' Em Rota'],
    'entregue':      ['badge-green',  getIcon('check') + ' Entregue'],
    'cancelada':     ['badge-red',    getIcon('x') + ' Cancelada'],
    // Motoristas
    'ativo':         ['badge-green',  getIcon('circle') + ' Ativo'],
    'inativo':       ['badge-red',    getIcon('circle') + ' Inativo'],
    // Veículos
    'disponivel':    ['badge-green',  getIcon('circle') + ' Disponível'],
    'manutencao':    ['badge-orange', getIcon('wrench') + ' Manutenção'],
    // Rotas
    'aberta':        ['badge-blue',   getIcon('refresh') + ' Aberta'],
    'em_andamento':  ['badge-yellow', getIcon('refresh') + ' Em Andamento'],
    'finalizada':    ['badge-green',  getIcon('check') + ' Finalizada'],
  };
...
function executeLogout() {
  ...
  // Limpa o campo de senha por segurança e avisa o usuário
...
async function loadCaches() {
  const [c, m, v] = await Promise.all([
    ...
    supabaseClient.from('motoristas').select('id,nome').eq('status','ativo').order('nome'),
    supabaseClient.from('veiculos').select('id,placa,modelo').eq('status','disponivel').order('placa'),
  ]);
...
async function loadDashboard() {
  ...
  try {
    ...
    const [all, pending, inRoute, done, drivers] = await Promise.all([
      supabaseClient.from('entregas').select('id', {count:'exact'}).gte('data_prevista', today),
      supabaseClient.from('entregas').select('id', {count:'exact'}).eq('status','pendente'),
      supabaseClient.from('entregas').select('id', {count:'exact'}).eq('status','em_rota'),
      supabaseClient.from('entregas').select('id', {count:'exact'}).eq('status','entregue'),
      supabaseClient.from('motoristas').select('id', {count:'exact'}).eq('status','ativo'),
    ]);
    ...
    const { data: mots } = await supabaseClient
      .from('motoristas')
    ...
    if (!mots || mots.length === 0) {
      ...
      mots.forEach(m => {
        motDiv.innerHTML += `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border2)">
          <div style="width:36px;height:36px;border-radius:50%;background:var(--primary-g);border:1px solid rgba(56,189,248,0.2);display:flex;align-items:center;justify-content:center;font-size:16px">${getIcon('user')}</div>
          <div style="flex:1">
...
async function loadEntregas(filters = {}) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon">${getIcon('package')}</div><p>Nenhuma entrega encontrada</p></div></td></tr>`;
      return;
    }
      ...
      tbody.innerHTML += `<tr>
        ...
        <td>
          <div class="table-actions">
            ${canEdit ? `
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openEntregaModal('${e.id}')" title="Editar">${getIcon('edit')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="changeStatus('${e.id}','${e.status}')" title="Mudar status">${getIcon('refresh')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="deleteEntrega('${e.id}')" title="Excluir">${getIcon('trash')}</button>
            ` : ''}
          </div>
        </td>
      </tr>`;
...
async function saveEntrega() {
  ...
  try {
    let error;
    if (id) {
      ({ error } = await supabaseClient.from('entregas').update(payload).eq('id', id));
    } else {
      ({ error } = await supabaseClient.from('entregas').insert(payload));
    }
    if (error) throw error;
...
async function saveStatus() {
  ...
  showLoading();
  const { error } = await supabaseClient.from('entregas').update({ status }).eq('id', id);
  hideLoading();
...
async function deleteEntrega(id) {
  ...
  showLoading();
  const { error } = await supabaseClient.from('entregas').delete().eq('id', id);
  hideLoading();
...
async function loadClientes(q = '') {
  ...
  try {
    let query = supabaseClient.from('clientes').select('*').eq('ativo', true).order('nome');
    ...
    tbody.innerHTML = '';
    if (!data || data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">${getIcon('users')}</div><p>Nenhum cliente encontrado</p></div></td></tr>`;
      return;
    }
    const canEdit = App.user.permissao !== 'visualizador';
    data.forEach(c => {
      tbody.innerHTML += `<tr>
        ...
        <td>
          <div class="table-actions">
            ${canEdit ? `
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openClienteModal('${c.id}')" title="Editar">${getIcon('edit')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="deleteCliente('${c.id}')" title="Excluir">${getIcon('trash')}</button>
            ` : ''}
          </div>
        </td>
      </tr>`;
...
async function deleteCliente(id) {
  ...
  showLoading();
  const { error } = await supabaseClient.from('clientes').update({ ativo: false }).eq('id', id);
  hideLoading();
  if (error) { toast('Erro ao excluir', 'error'); return; }
...
async function loadMotoristas(q = '') {
  ...
  try {
    let query = supabaseClient.from('motoristas').select('*').order('nome');
    if (q) query = query.ilike('nome', `%${q}%`);
    const { data, error } = await query;
    if (error) throw error;
    ...
    tbody.innerHTML = '';
    if (!data || data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><div class="empty-icon">${getIcon('user-cog')}</div><p>Nenhum motorista encontrado</p></div></td></tr>`;
      return;
    }
    const canEdit = App.user.permissao !== 'visualizador';
    data.forEach(m => {
      tbody.innerHTML += `<tr>
        ...
        <td>
          <div class="table-actions">
            ${canEdit ? `
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openMotoristaModal('${m.id}')">${getIcon('edit')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="deleteMotorista('${m.id}')">${getIcon('trash')}</button>
            ` : ''}
          </div>
        </td>
      </tr>`;
...
async function openMotoristaModal(id = null) {
  ...
  if (id) {
    ...
    showLoading();
    const { data } = await supabaseClient.from('motoristas').select('*').eq('id',id).single();
    hideLoading();
    if (data) {
...
async function saveMotorista() {
  ...
  try {
    let error;
    if (id) {
      ({ error } = await supabaseClient.from('motoristas').update(payload).eq('id', id));
    } else {
      ({ error } = await supabaseClient.from('motoristas').insert(payload));
    }
    if (error) throw error;
...
async function loadVeiculos(q = '') {
  ...
  try {
    let query = supabaseClient.from('veiculos').select('*').order('placa');
    ...
    tbody.innerHTML = '';
    if (!data || data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">${getIcon('truck')}</div><p>Nenhum veículo encontrado</p></div></td></tr>`;
      return;
    }
    const canEdit = App.user.permissao !== 'visualizador';
    data.forEach(v => {
      tbody.innerHTML += `<tr>
        ...
        <td>
          <div class="table-actions">
            ${canEdit ? `
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openVeiculoModal('${v.id}')">${getIcon('edit')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="deleteVeiculo('${v.id}')">${getIcon('trash')}</button>
            ` : ''}
          </div>
        </td>
      </tr>`;
...
async function loadRotas(q = '') {
  ...
  try {
    ...
    tbody.innerHTML = '';
    if (!data || data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">${getIcon('map')}</div><p>Nenhuma rota encontrada</p></div></td></tr>`;
      return;
    }
    const canEdit = App.user.permissao !== 'visualizador';
    data.forEach(r => {
      tbody.innerHTML += `<tr>
        ...
        <td>
          <div class="table-actions">
            ${canEdit ? `
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openRotaDetail('${r.id}')" title="Detalhes/Entregas">${getIcon('clipboard')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="openRotaModal('${r.id}')" title="Editar">${getIcon('edit')}</button>
            <button class="btn btn-ghost btn-icon btn-sm" onclick="deleteRota('${r.id}')" title="Excluir">${getIcon('trash')}</button>
            ` : `<button class="btn btn-ghost btn-icon btn-sm" onclick="openRotaDetail('${r.id}')">${getIcon('clipboard')}</button>`}
          </div>
        </td>
      </tr>`;
    });
...
async function openRotaModal(id = null) {
  ...
  const [mots, veics] = await Promise.all([
    supabaseClient.from('motoristas').select('id,nome').eq('status','ativo').order('nome'),
    supabaseClient.from('veiculos').select('id,placa,modelo').order('placa'),
  ]);
  ...
  if (id) {
    ...
    showLoading();
    const { data } = await supabaseClient.from('rotas').select('*').eq('id',id).single();
    hideLoading();
    if (data) {
...
async function saveRota() {
  ...
  try {
    let error, savedData;
    if (id) {
      ({ error } = await supabaseClient.from('rotas').update(payload).eq('id', id));
      savedData = { id };
    } else {
      const res = await supabaseClient.from('rotas').insert(payload).select().single();
      error = res.error;
      savedData = res.data;
    }
    if (error) throw error;
...
async function openRotaDetail(rotaId) {
  ...
  try {
    const [rotaRes, entregasRes] = await Promise.all([
      supabaseClient.from('rotas').select('*,motoristas(nome),veiculos(placa,modelo)').eq('id', rotaId).single(),
      supabaseClient.from('entregas').select('id,numero_nf,destino,cidade,uf,status,clientes(nome)').eq('rota_id', rotaId),
    ]);
    ...
    if (entregas.length === 0) {
      ...
      entregas.forEach(e => {
        entHtml += `<tr>
          ...
          <td>${e.cidade}/${e.uf}</td>
          <td>${statusBadge(e.status)}</td>
          <td><button class="btn btn-ghost btn-icon btn-sm" onclick="desvincularEntrega('${e.id}','${rotaId}')" title="Desvincular">${getIcon('x')}</button></td>
        </tr>`;
      });
      entHtml += `</tbody></table>`;
    }
    ...
    const { data: livres } = await supabaseClient
      .from('entregas')
...
async function vincularEntrega() {
  ...
  showLoading();
  const { error } = await supabaseClient.from('entregas').update({ rota_id: rotaId, status: 'em_rota' }).eq('id', entregaId);
  hideLoading();
...
async function desvincularEntrega(entregaId, rotaId) {
  ...
  showLoading();
  const { error } = await supabaseClient.from('entregas').update({ rota_id: null, status: 'pendente' }).eq('id', entregaId);
  hideLoading();
...
async function finalizarRota() {
  ...
  try {
    await supabaseClient.from('rotas').update({ status: 'finalizada' }).eq('id', rotaId);
    await supabaseClient.from('entregas').update({ status: 'entregue' }).eq('rota_id', rotaId).eq('status','em_rota');
...
async function copiarResumoRota() {
  ...
  try {
    const [rotaRes, entRes] = await Promise.all([
      supabaseClient.from('rotas').select('nome,data,motoristas(nome),veiculos(placa,modelo)').eq('id', rotaId).single(),
      supabaseClient.from('entregas').select('numero_nf,destino,cidade,uf,clientes(nome),status').eq('rota_id', rotaId),
    ]);
    const rota = rotaRes.data;
...
async function deleteRota(id) {
  ...
  showLoading();
  await supabaseClient.from('entregas').update({ rota_id: null }).eq('rota_id', id);
...
async function loadHistorico() {
  ...
  try {
    const [rotasRes, entRes] = await Promise.all([
      supabaseClient.from('rotas').select('id,nome,data,status,motoristas(nome),veiculos(placa)').eq('status','finalizada').order('data', { ascending: false }).limit(20),
      supabaseClient.from('entregas').select('id,numero_nf,destino,cidade,uf,status,data_prevista,clientes(nome)').in('status',['entregue','cancelada']).order('created_at', { ascending: false }).limit(30),
    ]);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (async () => {
    const { data } = await supabaseClient.from('motoristas').select('id,nome').eq('status','ativo').order('nome');
